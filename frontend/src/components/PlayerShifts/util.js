export function findAverageShiftLength(shifts) {
  const totalSeconds = shifts.reduce(
    (total, shift) => total + convertStringToSeconds(shift.shift.duration),
    0
  );
  ...
  return convertSecondsToStringMinutes(
    Math.round(totalSeconds / shifts.length)
  );
}
...
export function findStartingLineupOccurances(shifts) {
  const starting = shifts.filter(
    (shift) => shift.shift.period === 1 && shift.shift.startTime === "00:00"
  );
  return starting.length;
}

export function findFinalShiftOccurances(shifts) {
  const final = shifts.filter(
    (shift) => shift.shift.period === 3 && shift.shift.endTime === "20:00"
  );
  return final.length;
}
